		...
		for _, element := range test_table {
			t.Logf("\tChecking address %d", element.address)
			{
				if linear_memory.Data[element.address] != element.value {
					t.Errorf("\t\tThe address %d should have %d, intead it got %d", element.address, element.value, linear_memory.Data[element.address])
				}else {
					t.Logf("\t\tThe memory address %d had the correct value %d", element.address, linear_memory.Data[element.address])
				}
			}
		}
	}
