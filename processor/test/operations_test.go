...
func TestExecuteRTS(t *testing.T) {
	...
	var test_table = []struct {
	  ...
	  {0xFD, addrValue{0x01, 0xFF, 0x04}, addrValue{0x01, 0xFE, 0xFF}, processor.ProgramCounter{0x05, 0x00}, 0xFF},
	}
	t.Log("Given the need to test the ExecuteRTS.")
	{
	  for _, element := range test_table {
	    cpu.SP.Value = element.sp 
	    expected_pc = element.expected_pc
